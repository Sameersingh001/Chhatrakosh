...
export default function StudentLeavePage({ student }) {
  ...
  // Set studentId and fetch leaves when student prop changes
  useEffect(() => {
    if (student?._id) {
  ...
  // Fetch all leaves of the student
  ...
  const handleDelete = async (leaveId, teacherStatus, adminStatus) => {
    if (
      teacherStatus === "Recommended" ||
      adminStatus === "Approved" ||
      adminStatus === "Rejected"
    ) {
      alert("You cannot delete approved or recommended leaves.");
      return;
    }
    ...
    try {
      const res = await axios.post(`/api/student/leave/${leaveId}`, {
        withCredentials: true,
      });
      setMessage(res.data.message || "Leave deleted successfully!");
      setLeaves((prev) => prev.filter((leave) => leave._id !== leaveId));
    } catch (err) {
      console.error("❌ Failed to delete leave:", err);
      setMessage("Error deleting leave request");
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-blue-50 p-6 space-y-10">
      ...
      <div className="max-w-4xl mx-auto bg-indigo-100 shadow-lg rounded-3xl p-8 border border-indigo-200">
        ...
        <ul className="list-decimal pl-6 space-y-4 text-gray-800">
          <li>
            Fill in the <span className="font-bold text-indigo-800">reason</span>{" "}
            clearly and honestly.
          </li>
          <li>
            Choose <span className="font-bold text-indigo-800">accurate start and end dates</span>.
          </li>
          <li>
            Submit your request at least <span className="font-bold text-indigo-800">3 days before</span> the leave date.
          </li>
          <li>
            The leave request will first be <span className="font-bold text-green-700">Recommended by the Teacher</span> and then{" "}
            <span className="font-bold text-green-700">Approved by the Admin</span>. Track updates below.
          </li>
          <li>
          ...
          {leaves.length === 0 ? (
            ...
            <div className="overflow-x-auto rounded-xl">
              <table className="w-full text-left border-collapse border border-gray-200">
                ...
                <tbody>
                  {leaves.map((leave) => (
                    <tr key={leave._id} className="border-t hover:bg-green-50 transition">
                      <td className="px-5 py-3 max-w-[200px] break-words">{leave.reason}</td>
                      <td className="px-5 py-3">{leave.startDate}</td>
                      <td className="px-5 py-3">{leave.endDate}</td>
                      <td className={`px-5 py-3 font-medium ${leave.teacherStatus === "Recommended" ? "text-green-600" : leave.teacherStatus === "Not Recommended" ? "text-red-600" : "text-yellow-600"}`}>{leave.teacherStatus}</td>
                      <td className={`px-5 py-3 font-medium ${leave.adminStatus === "Approved" ? "text-green-600" : leave.adminStatus === "Rejected" ? "text-red-600" : "text-yellow-600"}`}>{leave.adminStatus}</td>
                      <td className="px-5 py-3">
                        <button
                          onClick={() => handleDelete(leave._id, leave.teacherStatus, leave.adminStatus)}
                          disabled={leave.adminStatus !== "Pending"}
                          className={`py-2 px-4 rounded-lg text-sm font-medium transition ${leave.adminStatus !== "Pending" ? "bg-gray-400 cursor-not-allowed text-white" : "bg-red-500 hover:bg-red-600 text-white"}`}
                        >
                          Delete
                        </button>
                      </td>
